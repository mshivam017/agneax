...
[Desktop Entry]
Name=Agneax
Comment=Agneax Desktop Session
Exec=/usr/bin/startagneax
Type=Application
LEOF

cat <<'LEOF' > /usr/bin/agneax-desktop
#!/usr/bin/env bash
# Agneax OS Desktop Launcher Executable
exec /opt/agneax/desktop/run.sh
LEOF
chmod +x /usr/bin/agneax-desktop

cat <<'LEOF' > /usr/bin/startagneax
#!/usr/bin/env bash
# Agneax OS Desktop Session Startup Script (Robust Exception Handled)
set -u

# Core Session Environment Initialization
export XDG_CURRENT_DESKTOP=Agneax
export DESKTOP_SESSION=Agneax
...
RUN_SCRIPT="/usr/bin/agneax-desktop"
...
if [ -f "agneax-services/target/release/agneax-core" ]; then
  cp "agneax-services/target/release/agneax-core" "$ROOTFS/usr/bin/agneax-core"
  chmod +x "$ROOTFS/usr/bin/agneax-core"
  echo "Rust Daemon installed successfully."
else
  echo "Warning: agneax-services/target/release/agneax-core not found. Skipping."
fi

# Copy built C++ helpers
mkdir -p "$ROOTFS/usr/lib"
if [ -f "agneax-shell/cpp_src/build/libcompositor_helper.so" ]; then
  cp "agneax-shell/cpp_src/build/libcompositor_helper.so" "$ROOTFS/usr/lib/libcompositor_helper.so"
  echo "C++ Layout Helper installed successfully."
fi

# Copy desktop and applications code
mkdir -p "$ROOTFS/opt/agneax/desktop"
mkdir -p "$ROOTFS/opt/agneax/settings"
mkdir -p "$ROOTFS/opt/agneax/installer"
mkdir -p "$ROOTFS/opt/agneax/branding"
mkdir -p "$ROOTFS/opt/agneax/file-manager"
mkdir -p "$ROOTFS/opt/agneax/terminal"
mkdir -p "$ROOTFS/opt/agneax/shell"
mkdir -p "$ROOTFS/opt/agneax/panel"
mkdir -p "$ROOTFS/opt/agneax/dock"
mkdir -p "$ROOTFS/opt/agneax/launcher"
mkdir -p "$ROOTFS/opt/agneax/system-monitor"

# Copy python launchers & components
cp "agneax-desktop/main.py" "$ROOTFS/opt/agneax/desktop/main.py" || true
cp "agneax-file-manager/file_manager.py" "$ROOTFS/opt/agneax/file-manager/file_manager.py" || true
cp "agneax-terminal/terminal.py" "$ROOTFS/opt/agneax/terminal/terminal.py" || true

# Copy settings, installer & branding
cp -R agneax-settings/* "$ROOTFS/opt/agneax/settings/" || true
cp -R agneax-installer/* "$ROOTFS/opt/agneax/installer/" || true
cp -R agneax-boot/branding/* "$ROOTFS/opt/agneax/branding/" || true

# Copy decoupled QML shell visual modules
cp -R agneax-shell/qml/* "$ROOTFS/opt/agneax/shell/" || true
cp -R agneax-panel/qml/* "$ROOTFS/opt/agneax/panel/" || true
cp -R agneax-dock/qml/* "$ROOTFS/opt/agneax/dock/" || true
cp -R agneax-launcher/qml/* "$ROOTFS/opt/agneax/launcher/" || true
cp -R agneax-system-monitor/qml/* "$ROOTFS/opt/agneax/system-monitor/" || true
...
cp -R agneax-shell/reference-source/* "$ROOTFS/opt/agneax/agneax-shell-source/" || true
...
chmod +x ./agneax-iso/package_store.sh
./agneax-iso/package_store.sh

# Generate PNG wallpaper from SVG vector for GRUB background (Step 2)
echo "Generating PNG wallpaper and text-based boot logo..."
rsvg-convert -w 1920 -h 1080 agneax-boot/branding/wallpaper.svg -o build/wallpaper.png || echo "Warning: Failed to generate wallpaper PNG using rsvg-convert"
...
cp -r agneax-login/* "$ROOTFS/usr/share/sddm/themes/agneax/"
...
# Copy configs (network, rules)
cp -R agneax-session/* "$ROOTFS/" || true
