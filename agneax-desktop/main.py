...
class SystemBridge(QObject):
    ...
    def launchApp(self, app_name):
        ...
        # Resolve path dynamically for dev/prod sibling structures
        base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        
        settings_path = os.path.join(base_dir, "agneax-settings", "main.py")
        if not os.path.exists(settings_path):
            settings_path = os.path.join(base_dir, "settings", "main.py")
            
        store_path = os.path.join(base_dir, "agneax-store", "main.py")
        if not os.path.exists(store_path):
            store_path = os.path.join(base_dir, "store", "main.py")
            
        installer_path = os.path.join(base_dir, "agneax-installer", "main.py")
        if not os.path.exists(installer_path):
            installer_path = os.path.join(base_dir, "installer", "main.py")
            
        terminal_path = os.path.join(base_dir, "agneax-terminal", "terminal.py")
        if not os.path.exists(terminal_path):
            terminal_path = os.path.join(base_dir, "terminal", "terminal.py")
            
        fm_path = os.path.join(base_dir, "agneax-file-manager", "file_manager.py")
        if not os.path.exists(fm_path):
            fm_path = os.path.join(base_dir, "file-manager", "file_manager.py")

        allowlist = {
            "control-center": ["python3", settings_path],
            "store": ["python3", store_path],
            "installer": ["python3", installer_path],
            "terminal": ["python3", terminal_path],
            "file-manager": ["python3", fm_path],
            "firefox": ["firefox"],
            "vlc": ["vlc"],
            "steam": ["steam"],
            "discord": ["discord"]
...
def main():
    ...
    # Load QML file
    qml_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "agneax-shell", "qml", "DesktopShell.qml"))
